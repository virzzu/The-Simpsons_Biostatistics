...
Para sintetizar esta tabla, vemos que el componente principal 1 (PC1) está fuertemente influenciado por los genes *POMC*, *MC4R*, *TMEM18* y *UHMK1*, genes que, según literatura, tienen un importante papel en la regulación central del balance energético y la saciedad. El componente princial 2 (PC2) está fuertemente influenciado por los genes *CADM2*, *NTRK2* y *UHMK1*, involucrados en la plasticidad neuronal y vías de señalización transmembrana. Finalmente, el componente principal 3 (PC3) tiene como genes clave *FTO*, *SH2B1*, *TMEM18* y *PHIP* (vemos que se repite *TMEM18* como en el PC1), genes que están relacionados con la obesidad y la respuesta a la insulina.
...
Para mejorar la normalidad y la interpretación de los coeficientes, ambas variables se transforman mediante logaritmo natural:
...
El Modelo 4 final incluye los terciles de los componentes principales, las variables demográficas edad y sexo, y el marcador clínico IL-6 logarítmico, permitiendo explorar tendencias de asociación, aunque las estimaciones son inestables debido al reducido tamaño del grupo con obesidad.
...
Tambien se pueden observar estos resultados en un grafico, para tener los datos más visuales. 
...
modelos <- list(
  ...
  "Modelo 2" = modelo2,
  "Modelo 3" = modelo3,
  "Modelo 4" = modelo4_il6 # ya no hace falta mirar más el modelo6
)

or_all <- lapply(names(modelos), function(nm) {
  tidy(modelos[[nm]], conf.int = TRUE, exponentiate = TRUE) %>%
    mutate(Modelo = nm) %>%
    select(Modelo, term, estimate, conf.low, conf.high)
}) %>%
  bind_rows() %>%
  rename(Predictor = term, OR = estimate, IC_low = conf.low, IC_high = conf.high)

ggplot(or_all, aes(x = OR, y = reorder(Predictor, OR), color = Modelo)) +
  geom_point(position = position_dodge(width = 0.7), size = 3) +
  geom_errorbarh(aes(xmin = IC_low, xmax = IC_high), 
                 height = 0.2, position = position_dodge(width = 0.7)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "black") +
  scale_x_log10() +
  labs(x = "Odds Ratio (IC 95%)", y = "", title = "Figura 13. Forest plot comparativo de OR entre modelos.") +
...
En este estudio se ha explorado la relación entre los perfiles de expresión génica y la obesidad mediante un enfoque combinado de análisis de componentes principales (PCA) y modelos de regresión logística. El PCA ha permitido identificar patrones globales de expresión génica y resumir la variabilidad de los genes en tres componentes principales. Se ha visto que ciertos genes clave, como POMC, MC4R, TMEM18, UHMK1, CADM2, NTRK2 y FTO, han contribuido de manera destacada a la variabilidad explicada por los primeros tres componentes. La categorización de estos componentes en terciles ha facilitado la descripción de tendencias de expresión génica, mostrando cómo algunos genes aumentan o disminuyen progresivamente entre los grupos, aunque estas observaciones deben considerarse exploratorias debido al tamaño limitado de la muestra.

Los modelos de regresión logística han evaluado la asociación entre los terciles de los componentes principales y la obesidad, ajustando progresivamente por variables demográficas, antropométricas, bioquímicas y de estilo de vida. Ninguno de los PCs ha mostrado asociaciones estadísticamente significativas con la obesidad, independientemente del nivel de ajuste. La inestabilidad de los modelos, reflejada en intervalos de confianza amplios, warnings de convergencia y estimaciones extremas de odds ratios, se atribuye principalmente al bajo número de individuos obesos en la muestra y a la presencia de separación completa en algunos predictores.

Entre las variables clínicas y bioquímicas evaluadas, la IL-6 log-transformada emerge como el marcador más consistente asociado a la obesidad, reflejando la relevancia de la inflamación sistémica de bajo grado en esta condición. Por el contrario, variables como glucosa y TNF-α han presentado problemas de convergencia o colinealidad, y medidas antropométricas adicionales, como circunferencia de cintura y cadera, no muestran asociaciones significativas cuando se incorporan en modelos ajustados, sugiriendo que su efecto podría estar mediado por la inflamación. Las variables dietéticas y de estilo de vida evaluadas tampoco aportan información predictiva adicional y, en algunos casos, su inclusión genera problemas de estabilidad en los modelos.

En conjunto, estos resultados indican que, dentro de la muestra estudiada, los patrones de expresión génica global capturados por los primeros tres componentes principales no predicen de manera independiente la obesidad. Sin embargo, la inflamación sistémica, representada por IL-6, destaca como un factor biológicamente relevante y consistente con la fisiopatología de la obesidad. Debido al bajo tamaño muestral y al desbalance entre grupos, las conclusiones deben interpretarse con carácter exploratorio. Este estudio proporciona una base para futuras investigaciones que incluyan muestras más grandes y balanceadas, permitiendo evaluar con mayor robustez las interacciones entre genética, inflamación y obesidad, y su potencial relevancia clínica.
